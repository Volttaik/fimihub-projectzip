...
export default function AvatarUploader({ userId, initialUrl, displayName, size = 56, onChange }: Props) {
  ...
  const onFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      if (!patchRes.ok) throw new Error((await patchRes.json()).error || 'Could not save avatar')
  ...
  return (
    ...
    >
      {url ? (
        <Image src={url} alt={displayName} fill sizes={`${size}px`} className="object-cover" />
      ) : (
        <span className="w-full h-full flex items-center justify-center bg-primary text-primary-foreground font-bold" style={{ fontSize: size * 0.36 }}>
